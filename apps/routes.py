...
@api.route('/api/<idx>',methods=['GET','POST','PUT','DELETE'])
...
def courses(term=None):
    if term is None:
        term = "Spring 2019"
    classes = Course.objects.order_by("+courseID")  # + for ascend
    return render_template("courses.html", courseData=classes, courses=True, login=False, term=term)
...
def enrollment():
    ...
    courseID = request.args.get('courseID')  # id = request.forms['courseID']
    courseTitle = request.args.get('title')  # forms work for both get and post
    user_id = session.get('user_id')
    if courseID:
        if Enrollment.objects(user_id=user_id, courseID=courseID):
            flash(f"You are already registered in {courseTitle}", "danger")
            return redirect(url_for("courses"))
        else:
            Enrollment(user_id=user_id, courseID=courseID).save()
            flash(f"Enrollment in {courseTitle} is done", "success")
    ...
    return render_template("enrollment.html", enrollment=True, title="Enrollement", classes=classes)
